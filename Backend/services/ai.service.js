...
const generateText = async (prompt, temperature = 0.7) => {
    ...
    try {
        ...
        throw new ApiError(502, "AI request has failed. Pleas wait a moment and try again.");
    }
};

export const generateLeadSummary = async (lead) => {
    const prompt = `You are and exper B2B sales analyst for a CRM called TTP CRM. Analyze the following sales lead and produce a concise assessment 
    
    Lead details:
    - Name: ${ lead.name || N/A }
    - Company: ${ lead.company || N/A }
    - Email: ${ lead.email || N/A }
    - Current pipeline stage: ${ lead.status || N/A }
    - Potential deal value: ${ lead.value || 0 }
    - Sources: ${ lead.source || "Unknown"}
    ...
    const schema = {
        ...
        properties: {
            summary: {
                type: "string",
                description: "2-3 sentences executive summary of lead",
            },
            riskScore: {
                type: "integer",
...
export const generateEmail = async ({lead, purpose, tone, sender }) => {
    ...
    Use line breaks (\\n) in the body. Keep it under 180 words. Sign off as ${sender?.name || "the TTP team"}.`;
...
export const generateSalesInsights = async (pipelineStats) => {
    ...
    const schema = {
        ...
        properties: {
            headline: {
                type: "string",
                description: "One sentence summary of the pipeline health",
            },
            insights: {
                type: "array",
